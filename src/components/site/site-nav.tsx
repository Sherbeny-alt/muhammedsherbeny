...
export function SiteNav() {
  ...
  return (
    ...
    >
      ...
      <div
        className={`overflow-hidden border-t border-border/60 bg-ink/95 backdrop-blur-xl transition-[max-height,opacity] duration-500 md:hidden ${
          open ? "max-h-96 opacity-100" : "max-h-0 opacity-0"
        }`}
      >
        <nav aria-label="Sections (mobile)" className="flex flex-col px-5 py-3">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
